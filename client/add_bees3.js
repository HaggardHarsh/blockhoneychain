...
const beePropsIndex = cinematicSource.indexOf('interface BeeProps {');
const heroIndex = cinematicSource.indexOf('function Hero({');
const beeComponents = cinematicSource.slice(beePropsIndex, heroIndex);

const scrollBeeMatch = cinematicSource.match(/function ScrollBee\(\) \{[\s\S]*?<\/div>\n  \);\n\}/);
const scrollBee = scrollBeeMatch[0];
